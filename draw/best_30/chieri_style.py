...
def draw_score_detail(
    ...
    write_constant = DataText(
        ...
        165,
        25,
        f"{song_score.constant:.1f}",
        StaticPath.kazesawa_regular,
    )
    text_overlay = draw_text(text_overlay, write_constant, average_color)
    write_rating = DataText(
        320, 170, 35, f"{song_score.rating:.3f}", StaticPath.exo_medium, anchor="lb"
    )
...
def draw_user_b30(data: UserBest30, language: str):
    ...
    write_arcaea_id = DataText(
        380, 364, 80, f"ID:{arcaea_id}", StaticPath.exo_medium, anchor="lb"
    )
    B30_bg = draw_text(B30_bg, write_arcaea_id)
    write_r10 = DataText(
        800, 560, 80, f"R10: {recent:.3f}", StaticPath.exo_medium, anchor="lb"
    )
    B30_bg = draw_text(B30_bg, write_r10)
    write_b30 = DataText(
        150, 560, 80, f"B30: {best:.3f}", StaticPath.exo_medium, anchor="lb"
    )
    B30_bg = draw_text(B30_bg, write_b30)
    write_theory = DataText(
        1400, 560, 80, f"Max: {theory:.3f}", StaticPath.exo_medium, anchor="lb"
    )
    B30_bg = draw_text(B30_bg, write_theory)
    # Score Info
